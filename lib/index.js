  ...
  if (extensions !== false) {
    ...
    if (source.include) {
      ...
      function walk(value) {
        ...
        /* istanbul ignore else */
        if (value.props) {
          _merge(props, value.props);
        }

        /* istanbul ignore else */
        if (value.methods) {
          _merge(methods, value.methods);
        }

        _merge(obj, value, _protected);

        _array(value.include).forEach(inc => {
          _array(inc.extensions).forEach(ext => {
            walk(ext.mixins);
          });

          _array(inc.mixins).forEach(mix => {
            const def = typeof mix === 'function'
              ? mix.call(inc, obj)
              : mix;

            if (Array.isArray(def)) {
              def.forEach(x => x && walk(x));
            } else if (def) {
              walk(def);
            }
          });
