...
function _super(chain, className) {
  ...
  debug('CHAIN %s <{ %s }>', className, Object.keys(chain[chain.length - 1]).join(', '));
...
function _new(obj) {
  ...
  for (let i = 0; i < _initializers.length; i += 1) {
    ...
    while (_factory) {
      ...
      if (Array.isArray(_retval)) {
        _factory = null;
        _extensions.push(_retval.map(x => {
          if (typeof x.mixins === 'function') {
            _initializers.push(x.mixins);
          } else if (Array.isArray(x.mixins)) {
  ...
  mergeDefinitionsInto.call(target, target, {
    extensions: _flatten(_extensions),
  }, obj.name || 'Object');
