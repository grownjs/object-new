...
function _extend(obj, source, extensions) {
  ...
  if (extensions !== false) {
    ...
    if (source.include) {
      ...
      const mixup = _chain(obj, _statics, true);

      mixup.props = _chain(obj, props);
      mixup.methods = _chain(obj, methods);

      obj.extensions[offset] = mixup;
    } else {
      // append definitions
