...
function _super(chain, className) {
  ...
  if (chain[chain.length - 1] && chain[chain.length - 1].init) {
    hiddenProperty(target, 'init', chain[chain.length - 1].init.bind(this));
  }
...
function _new(obj) {
  ...
  const _init = obj.init || (obj.extensions[obj.extensions.length - 1] || {}).init;

  /* istanbul ignore else */
  if (_init) {
    const retval = _init.apply(target, Array.prototype.slice.call(arguments, 1));

    /* istanbul ignore else */
    if (Object.prototype.toString.call(retval) === '[object Object]') {
      /* istanbul ignore else */
      if (retval.props) {
        mergePropertiesInto(target, retval.props);
      }

      /* istanbul ignore else */
      if (retval.methods) {
        mergeMethodsInto.call(target, target, retval.methods);
      }
