...
// factory
function _new(obj) {
  debug('INSTANCE <%s>', obj.class || obj.name || 'Object');
  ...
  mergeDefinitionsInto.call(target, target, obj, obj.class || obj.name || 'Object');
  ...
  function push(x) {
    ...
    /* istanbul ignore else */
    if (Array.isArray(x)) {
      Array.prototype.push.apply(_initializers, _flatten(x));
    }
  }
  ...
  if (_extensions.length) {
    ...
    mergeDefinitionsInto.call(target, target, mix, obj.class || obj.name || 'Object');
  }
...
module.exports = function $new(id, props, context, extensions) {
  ...
  if (props) {
    ...
    if (extensions !== false) {
      ...
      /* istanbul ignore else */
      if (props.extensions) {
        props.extensions.forEach(mix => {
          Array.prototype.push.apply(context.extensions, _flatten(mix));
        });
      }

      /* istanbul ignore else */
      if (props.mixins) {
        context.mixins = Array.isArray(props.mixins)
          ? props.mixins.slice()
          : props.mixins;
      }
