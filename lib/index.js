...
function _chain(target) {
  ...
  const source = arguments[1] || target;

  Object.keys(source).forEach(key => {
    const _source = _prop(source, key);
    ...
    if (!Object.prototype.hasOwnProperty.call(target, key)) {
      updateProperty(target, key, _source);
    } else {
  ...
  Object.keys(source).forEach(key => {
    ...
    if (deepMerge && Object.prototype.hasOwnProperty.call(obj, key)) {
      ...
      if (typeof d.value === 'function') {
        ...
        } else if (obj[key].indexOf(d.value) === -1) {
...
function _include(obj, extensions) {
  ...
  hiddenProperty(obj, 'props', _chain(props), true);
  hiddenProperty(obj, 'methods', _chain(methods), true);
  hiddenProperty(obj, 'init', _chain(_initializers), true);
...
function _extend(obj, source, extensions) {
  ...
  if (extensions !== false) {
    ...
    // append definitions?
    obj.extensions.push(_merge({}, source, ['include']));
  }
...
function _combine(ctx, data, parent) {
  ...
  if (parent) {
    ...
    hiddenProperty(ctx, 'init', parent.init, true);
    hiddenProperty(ctx, 'props', _reduce(parent.props, data.props), true);
    hiddenProperty(ctx, 'methods', _reduce(parent.methods, data.methods), true);
  }
...
function _proxy(id, ctor, parent) {
  ...
  hiddenProperty($proxy, 'props', Object.create(null), true);
  hiddenProperty($proxy, 'methods', Object.create(null), true);
...
module.exports = function $new(id, props, context, extensions) {
  ...
  if (props) {
    ...
    if (extensions !== false) {
      ...
      hiddenProperty(props, 'self', context);
