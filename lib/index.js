...
function _merge(obj, source, except, deepMerge) {
  Object.keys(source).forEach(key => {
    ...
    if (deepMerge && RE_CLASS.test(key)) {
      _update(obj, key, _prop(source, key));
      return;
    }
    ...
    if (deepMerge && key.charAt() !== '_') {
      ...
      } else if (typeof d.value === 'function' && (key.charAt() === '$' || key === 'init')) {
...
function _chain(target) {
  ...
  if (Array.isArray(target)) {
    const _methods = target.filter(x => typeof x === 'function');

    return function $chain() {
      const _values = [];

      for (let i = 0; i < _methods.length; i += 1) {
        const retval = _methods[i].apply(this, arguments);
  ...
  Object.keys(target).forEach(key => {
    ...
    if (_source.configurable) {
      ...
      /* istanbul ignore else */
      if (typeof _source.value !== 'undefined') {
    ...
    if (Object.prototype.toString.call(value) === '[object Object]') {
      ...
      if (value.include) {
        if (Array.isArray(value.include)) {
          value.include.forEach(x => {
            walk(typeof x === 'function' && x.length === 0
              ? x()
              : x);
          });
        } else {
...
function mergeMethodsInto(target, sourceMethods) {
  Object.keys(sourceMethods).forEach(prop => {
    ...
    _update(target, prop, {
      ...
      value: sourceMethods[prop].bind(this),
    });
  });
}
...
function _combine(ctx, data, parent) {
  ...
  if (parent) {
    ...
    _hidden(ctx, 'init', _chain([parent.init, ctx.init]), true);
    _hidden(ctx, 'props', _reduce(parent.props, data.props, ctx.props), true);
    _hidden(ctx, 'methods', _reduce(parent.methods, data.methods, ctx.methods), true);
  }
...
function _proxy(id, ctor, parent) {
  function $proxy(_id, props) {
    ...
    const instance = $proxy.new.apply(null, arguments);
...
module.exports = function $new(id, props, context, extensions) {
  ...
  while (keys.length) {
    ...
    if (typeof context[key] === 'undefined') {
      ...
      context[key] = (extensions !== false && _proxy(key, _new, _keys.join('.'))) || Object.create(null);
    }
...
// export all helpers
module.exports.cloneValue = _clone;
