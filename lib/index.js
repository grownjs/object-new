...
function _super(chain, className) {
  ...
  const _ctor = chain[chain.length - 1]
    ? chain[chain.length - 1]
    : null;

  /* istanbul ignore else */
  if (_ctor && typeof _ctor.init === 'function') {
    hiddenProperty(target, 'init', _ctor.init.bind(this));
  }
...
function _new(obj) {
  ...
  const ctor = obj.extensions[obj.extensions.length - 1].init;

  /* istanbul ignore else */
  if (ctor) {
    ctor.apply(target, Array.prototype.slice.call(arguments, 1));
  }
