...
function _super(chain, className) {
  ...
  // inherit init()
  hiddenProperty(target, 'init', mergeDefinitionsInto(target, {
    extensions: chain,
  }));
  ...
  /* istanbul ignore else */
  if (chain.length > 1) {
    _proxy = _super(chain.slice(0, chain.length - 1), className);
  }
...
function _new(obj) {
  ...
  if (obj.extensions && obj.extensions.length > 1) {
    hiddenProperty(target, 'super', _super(obj.extensions.slice(0, obj.extensions.length - 1), obj.name || 'Object'));
  }
