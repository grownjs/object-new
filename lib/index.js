...
const _protected = ['new', 'name', 'init', 'self', 'super', 'props', 'mixins', 'methods', 'include', 'extensions'];
  ...
  if (Array.isArray(props)) {
    return function $chain() {
      ...
      for (let i = 0; i < props.length; i += 1) {
        const retval = props[i].apply(this, arguments);

        /* istanbul ignore else */
        if (retval === null || typeof retval === 'string' || typeof retval === 'number' || typeof retval === 'boolean') {
          _return = _return || retval;
        }

        _values.push(retval);
      }

      return _return || _values.filter(x => x);
    };
  }

  Object.keys(props).forEach(key => {
    const d = _prop(props, key);

    if (Array.isArray(d.value) && d.value.length && d.value.filter(x => x).every(x => typeof x === 'function')) {
      d.value = _chain.call(this, obj, d.value.filter(x => x));

      updateProperty(props, key, d);
...
// merge objects
function _merge(obj, source, except, collect) {
  Object.keys(source).forEach(key => {
    /* istanbul ignore else */
    if (collect && RE_CLASS.test(key)) {
      return;
    }
    ...
    /* istanbul ignore else */
    if (collect && Object.prototype.hasOwnProperty.call(obj, key)) {
...
function _extend(obj, source, extensions) {
  ...
  if (extensions !== false) {
    ...
    if (source.include) {
      ...
      function walk(value, included) {
        while (typeof value === 'function' && !(value.class && value.new)) {
        ...
        if (Array.isArray(value)) {
          return value.forEach(x => walk(x, included));
        }

        /* istanbul ignore else */
        if (Object.prototype.toString.call(value) === '[object Object]') {
          walk(value.extensions);
          walk(value.mixins, true);
          ...
          if (value.init) {
            if (!_statics.init) {
              _statics.init = value.init;
            } else if (!Array.isArray(value.init)) {
              _statics.init = [value.init];
            } else {
              _statics.init.push(value.init);
            }
          }
      ...
      // override last applied extension
      const offset = Math.max(0, obj.extensions.length - 1);
      ...
      Object.keys(_statics).forEach(key => {
        ...
        if (Array.isArray(d.value) && d.value.length && d.value.filter(x => x).every(x => typeof x === 'function')) {
          obj[key] = source[key] || _chain.call(obj, obj, d.value.filter(x => x));
        } else if (typeof d.value === 'function') {
          if (Array.isArray(_statics[key]) && _statics[key].filter(x => x).every(x => typeof x === 'function')) {
            obj[key] = _chain.call(obj, obj, _statics[key]);
          } else {
            obj[key] = d.value;
          }
        } else if (d.get || d.set) {
          console.log('TODO');
        } else if (Array.isArray(_statics[key]) && _statics[key].length && _statics[key].filter(x => x).every(x => typeof x === 'function')) {
          obj[key] = _chain.call(obj, obj, _statics[key].filter(x => x));
        } else {
          obj[key] = source[key] || _statics[key];
        }
      });

      obj.extensions[offset] = {
        mixins: [],
        props,
        methods,
      };
      ...
      if (obj.mixins) {
        obj.mixins.splice(0, 1, _mixins.filter(x => x));
      } else {
        obj.mixins = _mixins.filter(x => x);
      }
...
function _combine(ctx, props) {
  ...
  ['mixins', 'include'].forEach(prop => {
    /* istanbul ignore else */
    if (props[prop]) {
      if (!ctx[prop]) {
        hiddenProperty(ctx, prop, [].concat(props[prop]).filter(x => x));
      } else {
        [].concat(props[prop]).filter(x => x).forEach(mix => {
          ctx[prop].push(mix);
        });
      }
    }
...
function _proxy(id, ctor, parent) {
  function $proxy(_id, props) {
    ...
    if (!(this instanceof $proxy)) {
      ...
      if ($child.mixins) {
        $child.mixins.splice(0, $child.mixins.length, $proxy.mixins);
      } else {
        $child.mixins = ($proxy.mixins || []).slice();
      }
...
function _new(obj) {
  ...
  for (let i = 0; i < _initializers.length; i += 1) {
    ...
    const _retval = typeof init.callback === 'function'
      ? init.callback.apply(init.context || target, (init.context ? [target] : []).concat(args))
      : init.callback;
