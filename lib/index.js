...
function _invoke(obj, value) {
  obj.extensions.push({
    init() {
      return Function.prototype.bind.apply(value, [null].concat(Array.prototype.slice.call(arguments)));
    },
  });
}
...
function readOnlyProperty(obj, prop, value, options) {
  ...
  updateProperty(obj, prop, {
    configurable: false,
    enumerable: options.hiddenProperty !== false
      ? prop.charAt() !== '_'
      : false,
    get() {
      /* istanbul ignore else */
...
function mergeMethodsInto(target, sourceMethods) {
  Object.keys(sourceMethods).forEach(prop => {
    ...
    readOnlyProperty(target, prop, sourceMethods[prop].bind(target), {
      isMethod: true,
    });
  });
}
...
function mergeDefinitionsInto(target, definition, objectName) {
  ...
  return function _init() {
    try {
      ...
      throw new Error(`Invalid '${objectName}.init' definition: ${e.message}`);
    }
  };
}
...
function _proxy(id, ctor, parent) {
  ...
  hiddenProperty($proxy, 'class', parent, true);
...
function _new(obj) {
  ...
  /* istanbul ignore else */
  if (obj.extensions && obj.extensions.length > 1) {
    const chain = obj.extensions.slice(0, obj.extensions.length - 1);

    let _key = chain.length - 1;
    let _ctx = target;
    let _proxy;
    ...
    while (_key >= 0) {
      ...
      for (let i = 0; i <= _key; i += 1) {
        /* istanbul ignore else */
        if (chain[i].methods) {
          mergeMethodsInto(_proxy, chain[i].methods);
        }
      }

      hiddenProperty(_ctx, 'super', _proxy);
  ...
  if (typeof ctor === 'function') {
    const retval = ctor.apply(target, Array.prototype.slice.call(arguments, 1));
