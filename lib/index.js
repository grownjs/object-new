...
function _chain(obj, props) {
  ...
  Object.keys(props).forEach(key => {
    ...
    if (Array.isArray(d.value) && d.value.filter(x => x).every(x => typeof x === 'function')) {
    ...
    if (source.include) {
      ...
      Object.keys(_statics).forEach(key => {
        ...
        if (Array.isArray(d.value) && d.value.filter(x => x).every(x => typeof x === 'function')) {
          ...
          console.log('TODO');
        } else if (Array.isArray(_statics[key]) && _statics[key].filter(x => x).every(x => typeof x === 'function')) {
      ...
      if (obj.mixins) {
        Array.prototype.push.apply(obj.mixins, _mixins.filter(x => x));
      } else {
