...
function _chain(obj, props) {
  if (Array.isArray(props)) {
    return function $chain() {
      ...
      for (let i = 0; i < props.length; i += 1) {
        const retval = props[i].apply(obj, arguments);
  ...
  Object.keys(props).forEach(key => {
    ...
    if (Array.isArray(d.value) && d.value.every(x => typeof x === 'function')) {
      d.value = _chain(obj, d.value);
  ...
  if (extensions !== false) {
    ...
    if (source.include) {
      ...
      Object.keys(_statics).forEach(key => {
        ...
        if (Array.isArray(d.value) && d.value.every(x => typeof x === 'function')) {
          obj[key] = source[key] || _chain(obj, d.value);
        } else if (typeof d.value === 'function') {
          obj[key] = _chain(obj, _statics[key], true);
        } else if (d.get || d.set) {
          console.log('TODO');
        } else {
      ...
      obj.extensions[offset] = {
        mixins: [],
        props: _chain(obj, props),
        methods: _chain(obj, methods),
      };
