...
function mergePropertiesInto(target, sourceProperties) {
  Object.keys(sourceProperties).forEach((prop) => {
    const originalValue = sourceProperties[prop];
    ...
    updateProperty(target, prop, {
      ...
      get() {
        /* istanbul ignore else */
        if (isFunction && (!originalValue.length || typeof _value === 'undefined')) {
          _value = originalValue.call(target);
        }
      ...
      set(value) {
        ...
        if (isFunction) {
          ...
          _value = originalValue.call(target, value, _value);
        } else {
          _value = value;
        }
      },
...
function _new(id, obj, props) {
  ...
  if (typeof obj.init === 'function') {
    const _obj = obj.init.apply(target, Array.prototype.slice.call(arguments, 2));
