...
function _super(chain, className) {
  ...
  debug('CHAIN %s (%s)', className, chain.length);

  let _proxy;

  /* istanbul ignore else */
  if (chain.length > 1) {
    _proxy = _super.call(this, chain.slice(0, chain.length - 1), className);
  }
  ...
  /* istanbul ignore else */
  if (chain[chain.length - 1] && chain[chain.length - 1].init) {
    hiddenProperty(target, 'init', chain[chain.length - 1].init);
  }

  mergeDefinitionsInto(target, {
    extensions: chain,
  }, className);
...
function _new(obj) {
  ...
  const _parent = _super.call(target,
    obj.extensions.slice(0, obj.extensions.length - 1), obj.name || 'Object');

  hiddenProperty(target, 'super', _parent);

  mergeDefinitionsInto(target, obj, obj.name || 'Object');

  const _init = obj.extensions[obj.extensions.length - 1].init;

  /* istanbul ignore else */
  if (_init) {
    _init.apply(target, Array.prototype.slice.call(arguments, 1));
  }
