...
function _new(id, obj, props) {
  ...
  if (typeof obj === 'string') {
    ...
    // static new
    readOnlyProperty(Factory, 'new', _new.bind(null, null, Factory), true);

    /* istanbul ignore else */
    if (props) {
      Object.keys(props).forEach((key) => {
        /* istanbul ignore else */
        if (typeof Factory[key] === 'undefined') {
          readOnlyProperty(Factory, key, props[key]);
        }
      });
    }

    // override object identity
    updateProperty(Factory, 'name', {
      value: obj,
    });
  ...
  if (typeof obj.init === 'function') {
    const _obj = obj.init.call(null, target, props);
