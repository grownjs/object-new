...
function mergeDefinitionsInto(target, definition, objectName) {
  ...
  return function _init() {
    init.forEach((ctor) => {
      const factory = ctor.apply(target, arguments) || {};
...
function call(ctx, m) {
  const args = _slice.call(arguments, 2);

  return ctx.extensions.map((ext) => {
    if (typeof ext[m] === 'function') {
      return ext[m].apply(null, args);
    }
...
function apply(ctx, m, a) {
  return call.apply(null, Array.prototype.concat.apply([m], a));
}
...
function _proxy(id, ctor) {
  ...
  readOnlyProperty($proxy, 'call', call.bind(null, $proxy), true);
  readOnlyProperty($proxy, 'apply', apply.bind(null, $proxy), true);
...
function _new(obj) {
  ...
  if (typeof ctor === 'function') {
    ctor.apply(target, _slice.call(arguments, 1));
  }
  ...
  try {
    while (keys.length) {
      ...
      if (typeof context[key] === 'undefined') {
        readOnlyProperty(context, key, _proxy(key, _new), true);
      }
